...
def get_packet_length_sum(bloco):
    ...
    for i in bloco:
        soma+= len(i)
    retorno = soma/(tamanho_maximo_pkt*len(bloco))
    return str(retorno)
...
def get_IAT_sum(bloco):
    ...
    for i in range(1,len(bloco)):
        tempo =  bloco[i].time - bloco[i-1].time
        soma += tempo
    retorno = soma/(idle_timeout*len(bloco))
    return str(retorno)
...
def get_1st_quartile_IAT(bloco):
    ...
    # print("quartil {}, len {}".format(quartil,tamanho))
...
def get_header_length_sum(bloco):
    ...
    retorno = header_s/(header_max*len(bloco))
    return str(retorno)
