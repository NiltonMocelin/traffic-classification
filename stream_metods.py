...
data = pd.read_csv("30_stream_randv2.csv")
file_saida = open('saida_30_stream.txt', '+a')
# exit(0)
...
for i in range(0, cv):
    ...
    cm = confusion_matrix(y_test, y_pred_htc, labels=[0,1,2,3,4,5,6,7])
    disp = ConfusionMatrixDisplay(confusion_matrix=cm,
                                  display_labels=[0,1,2,3,4,5,6,7])
    disp.plot()
    plt.title('matrix htc')
    # plt.show()
    plt.savefig('imgs_matrix/30htc_it'+str(i)+'.png')
    ...
    cm = confusion_matrix(y_test, y_pred_arfc, labels=[0,1,2,3,4,5,6,7])
    disp = ConfusionMatrixDisplay(confusion_matrix=cm,
                                  display_labels=[0,1,2,3,4,5,6,7])
    
    disp.plot()
    plt.title('matrix arfc')
    # plt.show()
    plt.savefig('imgs_matrix/30arfc_it'+str(i)+'.png')
